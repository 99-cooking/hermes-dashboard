...
export async function GET() {
  ...
  const agents = AGENTS.map(agent => {
    ...
    const weekCount = agentActions.length > 0
      ? (db.prepare(
          `SELECT COUNT(*) as c FROM activity_log WHERE action IN (${placeholders}) AND ts > datetime('now', '-7 days')`
    ...
    const skillCounts = agentActions.length > 0
      ? db.prepare(
          `SELECT action, COUNT(*) as c FROM activity_log
           WHERE action IN (${placeholders}) AND ts > datetime('now', '-30 days')
    ...
    const recentActivity = agentActions.length > 0
      ? db.prepare(
          `SELECT id, ts, action, detail, result FROM activity_log
           WHERE action IN (${placeholders}) ORDER BY ts DESC LIMIT 10`
